...
### Filtering the data with filtering functions.

This, however, not the only way to filter data. We have now added a new called
filterList, which stores functions that filter based on block width. Here is
a toy example using GRanges.
...
To filter our data, we create a function and add it using the addRowFilter. we can get the filtered data using a call to the getData() method.  
...
blocks_measure$getData()
```

Although the data returned is filtered, the underlying data object is untouched. If we clear the filters using the clearRowFilters method, the original data is returned. 
...
We can also add more filters: when more than one filter is present the result is a logical and between the filter results. 
